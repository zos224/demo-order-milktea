...
const OrderPageManager = () => {
    ...
    const onDeleted = (deleted) => {
        if (deleted) {
            ...
            const neworders = orders.filter((p) => p.id != id)
            setorders(neworders)
        }
    }
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between">
                <h2 className="font-bold text-xl">Quản lý đơn hàng</h2>
            </div>
            {!loading ? (
                    ...
                    <tbody>
                        {orders.map((order, index) => (
                            <tr key={order.id} className={`bg-white ${index == orders.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
                                ...
                                <td className="px-6 py-4">
                                    <button onClick={() => {setOpenModal(!openModal); setDeleteLink("/api/order/" + order.id)}} className="hover:text-boxdark-2 hover:font-bold">Xóa</button>
                                </td>
                            </tr>
