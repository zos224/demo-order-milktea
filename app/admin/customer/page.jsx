    ...
    useEffect(() => {
        const fetchcustomer = async () => {
            ...
            if (res.ok) {
                ...
                const key = `${customer.name}-${customer.phone}-${customer.address}`;
                if (customerMap.has(key)) {
                    customerMap.get(key).buyCount += 1;
                } else {
                    customerMap.set(key, {
                    name: customer.name,
                ...
                const filterCustomer = Array.from(customerMap.values());
                filterCustomer.sort((a, b) => b.buyCount - a.buyCount);
                ...
                <table className="w-full text-sm text-center text-gray-500 dark:text-white">
                    ...
                    <tbody>
                        {customer.map((customer, index) => (
                            <tr key={customer.id} className={`bg-white ${index == customer.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
