...
const linkPage = () => {
    ...
    const onDeleted = (deleted) => {
        if (deleted) {
            ...
            const newtoppings = toppings.filter((e) => e.id != id)
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between">
                <h2 className="font-bold text-xl">Quản lý topping</h2>
                <button className="dark:bg-bodydark dark:text-black-2 text-black bg-white rounded-md px-3 py-2 shadow-md"><Link href={"/admin/topping/create"}>Thêm topping</Link></button>
            </div>
            {!loading ? (
                ...
                <table className="w-full text-sm text-center text-gray-500 dark:text-white">
                    ...
                    <tbody>
                        {toppings.map((tp, index) => (
                            <tr key={tp.id} className={`bg-white ${index == toppings.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
                                ...
                                <td className="px-6 py-4">
                                    {formatCurrencyVND(tp.price)}
                                </td>
                                <td className="px-6 py-4">
                                    <Link className="hover:text-boxdark-2 hover:font-bold" href={"/admin/topping/update/" + tp.id}>Cập nhật</Link>
                                </td>
                                <td className="px-6 py-4">
                                    <button onClick={() => {setOpenModal(!openModal); setDeleteLink("/api/link/" + tp.id)}} className="hover:text-boxdark-2 hover:font-bold">Xóa</button>
                                </td>
                            </tr>
